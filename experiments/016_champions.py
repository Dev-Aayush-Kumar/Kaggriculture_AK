"""E16 - controlled comparison of Phase-4 survivors against P0/B2.

P0 is unchanged B2. P1 is the E14 movement rule. P2 is the E15 sell-defer
rule. They are not stacked. 32 seeds, both seats, every distinct pairing.
"""
...
CHAMPS = {
    "P0": dict(BASE),
    "P1": dict(BASE, move_ev_enabled=True),
    "P2": dict(BASE, sell_defer_enabled=True),
}

PAIRINGS = (("P0", "P1"), ("P0", "P2"), ("P1", "P2"))
...
def main():
    ...
    print(f"E16 champion comparison: {len(PAIRINGS)} pairings x {len(SEEDS)} "
          f"seeds x 2 seats = {len(jobs)}\n", flush=True)
    records = H.run_jobs(jobs, progress=progress)
    base = H.save(records, "e16-champions")
