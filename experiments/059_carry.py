"""E59 - count carried wheat toward the feed buffer, pickup uncapped.

PICK1 showed that cutting wheat volume while adding shed trips loses to H4.
CARRY keeps H4's n_animals pickup and only stops BUY_PRODUCT restocks of
wheat that is already in someone's hands.

Hypothesis: phantom restocks are -EV once walking is unchanged, so counting
carried wheat raises P(win). If CARRY also loses, the extra wheat volume
itself is +EV and pickup-churn is not the leak.
"""
...
CARRY = dict(H4, feed_count_carried=True)
...
def summarize(title, records, label):
    ...
    print(f"  wheat h/sold/rev={wheat_h:.1f}/{wheat_s:.1f}/{wheat_r:,.0f}  "
          f"wool_r={wool_r:,.0f} milk_r={milk_r:,.0f}")
    print(f"  day1$={day1:.0f} buy_prod={buys:.1f} pickup/feed={pickups:.1f}/"
          f"{feeds:.1f} esc={escaped:.2f} drought={drought:.2f} bad={bad}")
    return {
...
def main():
    ...
    print(f"E59 carried-wheat buffer {stage}\n")
    if stage == "screen":
        cand_h4, h4 = run_pair("CARRY", CARRY, "H4", H4, SCREEN_SEEDS,
                               "e59-carry-h4")
        print()
        cand_p, p1s = run_pair("CARRY", CARRY, "P1S", P1S, SCREEN_SEEDS,
                               "e59-carry-p1s")
        print(f"\nCARRY screen: vs H4 {cand_h4['wins']}-{cand_h4['losses']}-"
              f"{cand_h4['ties']} p10 {cand_h4['p10']:.0f} vs {h4['p10']:.0f} "
              f"day1$ {cand_h4['day1']:.0f} vs {h4['day1']:.0f} "
              f"wheat_h/s {cand_h4['wheat_h']:.0f}/{cand_h4['wheat_s']:.0f} vs "
              f"{h4['wheat_h']:.0f}/{h4['wheat_s']:.0f} "
        ...
        if vs_h4 and vs_p:
            print("          KEEP")
            print("Run: python experiments/059_carry.py finals")
        else:
            print(f"          DROP  (vs H4 keep={vs_h4} vs P1S keep={vs_p})")
        return
    if stage == "finals":
        cand_h4, h4 = run_pair("CARRY", CARRY, "H4", H4, FINAL_SEEDS,
                               "e60-carry-h4")
        print()
        cand_p, p1s = run_pair("CARRY", CARRY, "P1S", P1S, FINAL_SEEDS,
                               "e60-carry-p1s")
        print(f"\nCARRY finals: vs H4 {cand_h4['wins']}-{cand_h4['losses']}-"
        ...
        if finals_promote(cand_h4, h4, cand_p, p1s):
            print("E60: CARRY beats H4 and P1-S without tail damage.")
        else:
            print("E60: CARRY does not replace H4.")
        return
