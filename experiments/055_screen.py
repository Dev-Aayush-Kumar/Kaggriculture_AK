"""E55 - screen day-0 third-sheep reserve and crop fertilizer use.

H4 and P1-S stay frozen. Candidates only change existing Config fields.

R280: livestock_reserve=280 (exactly enough for 3 sheep after hire+3 cows)
R0:   livestock_reserve=0
FERT: fertilize_crops=True (spend collected fertilizer on wheat)

Hypothesis: buying the third sheep on day 0 captures two extra wool events
and raises P(win). FERT tests whether wheat yield beats fertilizer sales.
"""
...
R280 = dict(H4, livestock_reserve=280)
R0 = dict(H4, livestock_reserve=0)
FERT = dict(H4, fertilize_crops=True)
...
def summarize(title, records, label):
    ...
    print(f"  wheat h/rev={wheat_h:.1f}/{wheat_r:,.0f}  wool={wool_h:.1f}/{wool_r:,.0f}"
          f"/{wool_f:.1f}  milk_r={milk_r:,.0f}  fert_r={fert_r:,.0f}")
    print(f"  max_ani={max_ani:.2f} first6={first6:.2f} escaped={escaped:.2f}"
    ...
    return {
        ...
        "escaped": escaped, "drought": drought, "wool_h": wool_h,
        "first6": first6, "max_ani": max_ani, "fert_r": fert_r, "wheat_h": wheat_h,
    }


def screen_keep(cand, opp):
    """Keep a candidate if it does not clearly lose P(win) or collapse p10."""
...
def main():
    ...
    if stage == "screen":
        ...
        for label, params in variants:
            ...
            print()
        print(f"E55 screen kept: {kept or 'none'}")
        if kept:
            print("Run: python experiments/055_screen.py finals")
        return
    if stage == "finals":
        # Survivors from the screen are passed as extra args, else R280.
        names = sys.argv[2:] or ["R280"]
        table = {"R280": R280, "R0": R0, "FERT": FERT}
        for label in names:
            params = table[label]
            cand_h4, h4 = run_pair(label, params, "H4", H4, FINAL_SEEDS,
                                   f"e56-{label.lower()}-h4")
            print()
            cand_p, p1s = run_pair(label, params, "P1S", P1S, FINAL_SEEDS,
                                   f"e56-{label.lower()}-p1s")
            ...
            if finals_promote(cand_h4, h4, cand_p, p1s):
                print(f"E56: {label} beats H4 and P1-S without tail damage.")
            else:
                print(f"E56: {label} does not replace H4.")
            print()
        return
